...
def main(args):
    ...
    epoch = 8
    task = 'oxygen'
    pt_path = f'./weights/downstream_tasks/train_attn_oxygen/len8000_attentionTrue/8_gene_attn_optim_model.pt'
    ...
    test_dataset = OxygenDataset(data_df, max_length=max_length)
    ...
    genome_ids, preds, probs = evaluate(model, test_loader, device)
    ...
    print(f"Predicted label distribution: {pd.Series(preds).value_counts()}")
