...
class AttentionPoolingModel(nn.Module):
    def __init__(self, dim=512, max_len=20000, use_attention=True, dropout=0.2):
        super().__init__()
    ...
    def forward(self, x, mask, return_attn=False):
        ...
        if self.use_attention:
            ...
            # pooled = torch.sum(attn_weights.unsqueeze(-1) * x, dim=1)  # mean pooling (B, D)
        else:
            mask_expanded = mask.unsqueeze(-1).float()  # (B, L, 1)
            x_masked = x * mask_expanded  # (B, L, D)
            lengths = mask.sum(dim=-1, keepdim=True).clamp(min=1)  # (B, 1)
            pooled = x_masked.sum(dim=1) / lengths  # mean pooling: (B, D)

        # combined = torch.mean(pooled, dim=1)  # (B, D)
        pooled = self.norm(pooled)
        out = self.fc(pooled)   # (B, 1)

        if return_attn and self.use_attention:
            return out, attn_weights
        else:
            return out

class TemperatureDataset(Dataset):
    def __init__(self, data, max_length=20000):
        self.data = data
...
def main(args):
    ...
    USE_ATTENTION = True
    epoch = 60
    pt_path = f'./weights/downstream_tasks/train_attn_temperature/len8000_attentionTrue/60_gene_attn_optim_model.pt'
    txt_path = './scripts/data/genome_unique_ids.csv'
    task = 'temperature'
    train_path = './scripts/data/split_meta_temperature_phylo.csv'
    output_folder = args.output_dir + f'/test_attn_{task}'
    os.makedirs(output_folder, exist_ok=True)
    train_data = pd.read_csv(train_path) 
    scaler = MinMaxScaler() 
    labels = train_data['label_x'].values.reshape(-1, 1)
    scaler.fit(labels)
    ...
    for _, seq_info in tqdm(sequences.iterrows(), total=len(sequences)):
        ...
        gene_emb_path = os.path.join(emb_path, genome_id,str(genome_id) + '.pkl')
        if os.path.exists(gene_emb_path):
            item = {}
            item['genome_id'] = genome_id
            item['emb_path'] = gene_emb_path
            data_set.append(item)

    data_df = pd.DataFrame(data_set)

    test_dataset  = TemperatureDataset(data_df, max_length=max_length)
    ...
    output_path = os.path.join(output_folder, f'predicted_{task}_{base_name}_results.csv')
    result_df.to_csv(output_path, index=False)
    
    print(f"Prediction results saved to: {output_path}")
    print(f"Predicted temperature range: {np.min(y_pred_denorm):.2f}°C ~ {np.max(y_pred_denorm):.2f}°C")
