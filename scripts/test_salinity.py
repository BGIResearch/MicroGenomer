...
def main(args):
    ...
    USE_ATTENTION = True
    epoch = 46
    pt_path = f'./weights/downstream_tasks/train_attn_salinity/len8000_attentionTrue/46_gene_attn_optim_model.pt'
    txt_path = './scripts/data/genome_unique_ids.csv'
    task = 'salinity'
    output_folder = args.output_dir + f'/test_attn_{task}'
    os.makedirs(output_folder, exist_ok=True)

    train_path = './scripts/data/split_meta_salinity_phylo.csv'
    ...
    test_dataset = SalinityDataset(data_df, max_length=max_length)
    ...
    genome_ids, y_pred = evaluate(model, test_loader, device)

    result_df = pd.DataFrame({
        'genome_id': genome_ids,
        f'predicted_{task}_value': y_pred
    })
    ...
    print(f"Predicted salinity range: {np.min(y_pred):.2f} ~ {np.max(y_pred):.2f}")
